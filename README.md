...
### Huh?
RG is a video downloader with a lot of features, most of which are filters for fine-tuning your search

### How to use
##### Python 3.11 or greater required
- RG is a cmdline tool, no GUI
- See `requirements.txt` for additional dependencies. Install with:
  - `python -m pip install -r requirements.txt`
##### Install as a module
- `cd rg`
- `python -m pip install .`
- `python -m rg [options...]` OR simply
- `rg [options...]`
##### Without installing
- `cd rg`
- Run either:
  - `python rg/__main__.py [options...]` OR
  - `rg.cmd [options...]` (Windows)
  - `rg.sh [options...]` (Linux)

- For bug reports, questions and feature requests use our [issue tracker](https://github.com/trickerer01/RG/issues)

#### Search
- RG provides advanced searching functionality (`rg pages`). Search is performed using extended website native API
...
- `rg_tags.list` file contains all existing tags for current version, same with `rg_cats.list` (categories) and `rg_arts.list` (artists) files. Any tag/category/artist you use is required to be valid and every `extra tag` needs to be a valid tag, category or artist. That is, unless you also utilize...
...
2. `--download-scenario` explained in detail:
  ...
  - *SCRIPT* always contains spaces hence has to be escaped by quotes:
    - rg ids \<args>... -script ***"***<NOTHING>sub1: tags1; sub2: tags2 ...***"***
...
3. Downloading a set of videos
  ...
  - Using id sequence:
    - Syntax: `--use-id-sequence` / `-seq`, `rg ids` only (or download scenario subquery)
...
7. Interrupt & resume
  ...
  - There is only one way to make a 'safe' interruption which is to tap `qg` quickly. This will signal the scanner to skip the rest of its queue and after all remaining downloads get processed the app will exit normally. Use `dg` sequence to also cancel all pending downloads, or `cg` to cause interruption of all active writes and immediate exit _(unfinished files may get deleted, according to current unfinished files policy)_
  ...
  - Continue file has to be used with `rg file` (see `using 'file' mode` above)
...
9. Scanning for unpublished posts
  ...
  - `rg ids` can automatically scan past maximum available post ID until it reaches the actual maximum post ID available.
  - Syntax: `-lookahead <AMOUNT>`. `<AMOUNT>` here is the number of sequential empty post IDs to assume the end of existing posts. Example:
    - `rg ids -start 3500000 -count 500 ... -lookahead 100` - scan until 3,500,500 and continue **indefinetely** until 100 post requests in a row return 'not found' error

#### Examples
1. Pages
  - All videos by a single tag:
    - `rg pages -pages 9999 -search_tag TAG1`
  - Up to 48 videos with both tags present from a single author in 1080p, save to a custom location:
    - `rg pages -pages 2 -path PATH -quality 1080p -search_art ARTIST -search_tag TAG1,TAG2`
  - Up to 24 videos on page 3 with any of 3 tags from any of 2 authors under any of 2 categories, exclude any kind of `vore` or `fart`, in best quality, with minimum score of 100 and minimum rating of 90%, use proxy, save to a custom location, save tags, log everything, use shortest names for files:
    - `rg pages -log trace -start 3 -pages 1 -path PATH -proxy https://127.0.0.1:222 -tdump -quality 2160p -minscore 100 -minrating 90 -search_cat CATEGORY1,CAT_EGORY2 -search_art ART_IST1,ARTIST2 -search_tag TAG1,TAG2,TAG3 -search_rule_cat any -search_rule_art any -search_rule_tag any -naming 0 -*vore -fart*`
  - All videos uploaded by a user, if tagged with either of 2 desired tags, in best quality, sorted into subfolders by several desired (known) authors, putting remaining videos into a separate folder, setup for interrupt & continue:
    - `rg pages -pages 9999 -path PATH --store-continue-cmdfile -quality 2160p -uploader USER_ID (TAG1~TAG2) -script "name1: AUTHOR1; name2: AUTHOR2; name3: AUTHOR3; rest: * -utp always"`
  - All videos from artist's page, if tagged with a desired tag, in best quality:
    - `rg pages -pages 999 -path PATH -quality 2160p -model ARTIST_NAME TAG1`

2. Ids
  - All existing videos in range:
    - `rg ids -start 3200000 -count 100`
    - `rg ids -start 3200000 -end 3200099`
  - You can use the majority of arguments from `pages` examples. The only argument that is unique to `rg ids` is `--use-id-sequence` (`-seq`), see above where it's explained in detail
...
1. Using `rg ids` to process greater id range
- `rg ids <args...> -start 1 -end 3500000`
...
- `rg ids <args...> -start 1 -end 235`
- `rg ids <args...> -start 3045050 -end 3100000`
- `etc.`

**Solution 2**: use `rg pages` (listed videos):
- `rg pages <args...> -start 1 -pages 9999`

2. Using `extra tags` to download only certain tag / category / artist / uploader
- `rg pages <args...> -start 1 -pages 9999 ARTIST`
- `rg ids <args...> -start ... -end ... (TAG1~TAG2)`
- `etc.`
...
- `rg pages <args...> -start 1 -pages 99 -search_art ARTIST`
- `rg pages <args...> -start 1 -pages 99 -search_tag TAG1,TAG2 -search_rule_tag any`

**Solution 2**: to search by uploader use `-uploader` argument (user video pages):
- `rg pages <args...> -start 1 -pages 99 -uploader USER_ID`
