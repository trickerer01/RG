...
from .defs import DOWNLOAD_MODE_TOUCH, FOLDER_INDEX_FILENAME, PREFIX, QUALITIES, QUALITY_480P, SEARCH_RULE_DEFAULT, SITE, Duration
...
class CmdTests(TestCase):
    ...
    def test_cmd_pages02(self):
        prepare_arglist(['pages', '-start', '2', '-pages', '1', '-uploader', '1234', '(2d~abs)', '--skip-empty-lists', '-script',
                         'a: 2d; b: 3d; c: ai -2d; d: * -utp always', '-naming', 'prefix|quality', '-log', 'warn'])
    ...
    def test_cmd_pages03(self):
        prepare_arglist(['pages', '-playlist_name', 'goods', '-start', '3', '-pages', '2', '-quality', '480p', '-dnoempty',
                         '-minscore', '12', '-continue', '-unfinish', '-tdump', '-ddump', '-cdump', '-sdump'])
        self.assertEqual('goods', Config.playlist_name)
    ...
    def test_cmd_ids02(self):
        prepare_arglist(['ids', '-start', '1000', '-end', '999', '(ai~abs)', '(2d~vhs)', '-dmode', 'touch', '--store-continue-cmdfile',
